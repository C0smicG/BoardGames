...
public class Reversi {
    ...
    private String CURRENT_PLAYER = "B";
    private List<List<Integer>> DIRECTIONS = new ArrayList<List<Integer>>();
    public Board GaemeBoard;


    public Reversi(int r, int c) {
        // constructor
        setDIRECTIONS();
        this.GaemeBoard = new Board(r, c);
        List<List<String>> state = this.GaemeBoard.getBoard();
        state.get((Integer) ((r / 2) - 1)).set((Integer) ((c / 2) - 1), BLACK_PLAYER);
        state.get((Integer) ((r / 2))).set((Integer) ((c / 2) - 1), WHITE_PLAYER);
        state.get((Integer) ((r / 2) - 1)).set((Integer) ((c / 2)), WHITE_PLAYER);
        state.get((Integer) ((r / 2))).set((Integer) ((c / 2)), BLACK_PLAYER);
    ...
    public String getWinner() {
        // returns the winner by comparing the number of black pieces vs white pieces
    ...
    public List<List<Integer>> getFlippedItems(int row, int col, String player) {
    ...
    // checks if current player has a valid move
    public boolean currentPlayerHasValidMove() {
        return hasValidMoves(this.CURRENT_PLAYER);
    }

    public void flipTiles(List<List<Integer>> flipList) {
